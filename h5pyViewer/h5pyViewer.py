...
class AboutFrame(wx.Frame):
    def __init__(self, parent):
        ...
        bmp = wx.StaticBitmap(panel, -1, wx.Bitmap(os.path.join(imgDir, 'splash1.png'), wx.BITMAP_TYPE_ANY),
                              (30, st0.Position[1] + st0.Size[1] + 10))
...
class HdfTreePopupMenu(wx.Menu):
    ...
    def OnShowRoiStat(self, event):
        ...
        hid = wxTree.GetPyData(wxNode)
        if type(hid) == tuple: hid = hid[0]  # external link->get dataset
        dlg = wx.FileDialog(wxTree, "Choose valid mask file (e.g. pilatus_valid_mask.mat)", os.getcwd(), '',
        ...
        dlg = wx.FileDialog(wxTree, "Choose ROI mask file (e.g. pilatus_integration_mask.mat)", os.getcwd(), '',
                            'MATLAB files (*.mat)|*.mat|all (*.*)|*.*', wx.FD_OPEN | wx.FD_CHANGE_DIR)
        if dlg.ShowModal() == wx.ID_OK:
...
class HdfViewerFrame(wx.Frame):
    ...
    def OnOpen(self, event):
        dlg = wx.FileDialog(self, "Choose a file", os.getcwd(), '',
                            'HDF5 files (*.hdf5;*.h5)|*.hdf5;*.h5|all (*.*)|*.*', wx.FD_OPEN | wx.FD_CHANGE_DIR)
        if dlg.ShowModal() == wx.ID_OK:
            path = dlg.GetPath()
    ...
    def BuildMenu(self):
        ...
        # -------- File Menu --------
        mn = wx.Menu()
        mnItem = mn.Append(wx.ID_OPEN, '&Open', 'Open a new document');
        self.Bind(wx.EVT_MENU, self.OnOpen, mnItem)
        # mnSub = wx.Menu()
        # mnItem=mnSub.Append(wx.ID_ANY, 'SubMenuEntry', 'My SubMenuEntry')
        # mn.AppendMenu(wx.ID_ANY, 'SubMenu', mnSub)
        mn.AppendSeparator()
        mnItem = mn.Append(wx.ID_EXIT, '&Quit', 'Quit the Application');
        self.Bind(wx.EVT_MENU, self.OnCloseWindow, mnItem)
        ...
        self.Bind(wx.EVT_MENU, self.OnAbout, mnItem)
