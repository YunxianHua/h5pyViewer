...
class MPLCanvasImg(FigureCanvas):
    ...
    def InitChild(self, data):
        ...
        vmin = np.min(msk);
        vmax = np.max(msk)
        vmin = max(vmin, avg - 3 * std);
        vmax = min(vmax, avg + 3 * std)
        if vmin == 0: vmin = 1
        if vmax <= vmin:
            vmax = vmin + 1
...
class DlgColBarSetup(wx.Dialog):
    def __init__(self, parent):
        ...
        colMapLst = (
        'hot', 'spectral', 'jet', 'gray', 'RdYlBu', 'hsv', 'gist_stern', 'gist_ncar', 'BrBG', 'RdYlBu', 'brg',
        'gnuplot2',
        'prism', 'rainbow',)
...
class HdfImageFrame(wx.Frame):
    ...
    def BuildMenu(self, dtype):
        ...
        # -------- Edit Menu --------
        mn = wx.Menu()
        mnItem = mn.Append(wx.ID_ANY, 'Setup Colormap', 'Setup the color mapping ');
        self.Bind(wx.EVT_MENU, self.OnColmapSetup, mnItem)
        mnItem = mn.Append(wx.ID_ANY, 'Invert X-Axis', kind=wx.ITEM_CHECK);
        self.Bind(wx.EVT_MENU, self.OnInvertAxis, mnItem)
        self.mnIDxAxis = mnItem.GetId()
        mnItem = mn.Append(wx.ID_ANY, 'Invert Y-Axis', kind=wx.ITEM_CHECK);
        self.Bind(wx.EVT_MENU, self.OnInvertAxis, mnItem)
        mnItem = mn.Append(wx.ID_ANY, 'Show Moments', 'Show image moments ', kind=wx.ITEM_CHECK);
        self.Bind(wx.EVT_MENU, self.OnShowMoments, mnItem)
        self.mnItemShowMoment = mnItem
        mnItem = mn.Append(wx.ID_ANY, 'Tomo Normalize',
                           'Multiplies each pixel with a normalization factor. Assumes there exist an array exchange/data_white',
                           kind=wx.ITEM_CHECK);
        self.Bind(wx.EVT_MENU, self.OnTomoNormalize, mnItem)
        self.mnItemTomoNormalize = mnItem

        if dtype == np.complex128:
            mnItem = mn.Append(wx.ID_ANY, 'Complex: Phase', kind=wx.ITEM_CHECK);
            self.Bind(wx.EVT_MENU, self.OnSetComplexData, mnItem)

        mnBar.Append(mn, '&Edit')
        mn = wx.Menu()
        mnItem = mn.Append(wx.ID_ANY, 'Help', 'How to use the image viewer');
        self.Bind(wx.EVT_MENU, self.OnHelp, mnItem)
    ...
    def OnShowMoments(self, event):
        if event.IsChecked():
            dlg = wx.FileDialog(self, "Choose valid mask file (e.g. pilatus_valid_mask.mat)", os.getcwd(), '',
                                'MATLAB files (*.mat)|*.mat|all (*.*)|*.*', wx.FD_OPEN | wx.FD_CHANGE_DIR)
