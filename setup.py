...
def getVersion():
    ...
    if os.access(fn, os.R_OK):
        ...
        # ver='0.0.0.'+str(ver)
        # gitcmt=res[0][:7]
        p = subprocess.Popen('git describe --match ''v*.*.*'' --long', shell=True, stdout=subprocess.PIPE,
...
def main():
    argv = sys.argv
    if 'debug' in argv:
        script_args = ['--name', '--version', '--fullname', '--author', '--author-email', '--maintainer',
                       '--maintainer-email', '--contact', '--contact-email', '--url', '--license', '--description',
                       '--long-description', '--platforms', '--classifiers', '--keywords', '--provides', '--requires',
                       '--obsoletes']
